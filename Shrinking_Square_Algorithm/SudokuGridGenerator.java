package Shrinking_Square_Algorithm;
...
class SudokuGridGenerator {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 500; i++) {
            ...
            // if(isPerfect(sudoku.generateGrid())) System.out.println("PERFECT GRID #" +
            // String.format("%,d",(trials)));

            // Print the sudoku grids to console
            // if(isPerfect(sudoku.generateGrid())) printGrid(sudoku.grid);
            if (!isPerfect(sudoku.generateGrid()))
                continue;

            // Save sudoku grids to txt file
            try {
                // String parent = new File(System.getProperty("user.dir")).getParent();
                String fileLocation = new File(System.getProperty("user.dir"))
                        + String.format("\\Sudoku_Grids\\grid_%04d.txt", (i + 1));
                
                // System.out.println(fileLocation);
                // System.out.println(getGrid(sudoku.grid));

                File f = new File(fileLocation);
                f.createNewFile();
                FileOutputStream fOutputStream = new FileOutputStream(f, false);
                PrintWriter printWriter = new PrintWriter(fOutputStream);

                printWriter.write(getGrid(sudoku.grid));
                printWriter.flush();
                fOutputStream.close();
                printWriter.close();
    ...
    public int[] generateGrid() {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            for (int a = 0; a < 2; a++) {
                ...
                boolean[] registered = new boolean[10]; // index 0 will intentionally be left empty since there are only
                                                        // number 1-9.
                int rowOrigin = i * 9;
                int colOrigin = i;
                                ...
                                for (int z = (a % 2 == 0 ? (i % 3 + 1) * 3 : 0); z < 9; z++) {
                                    ...
                                    if ((!sorted[scan] && !sorted[boxStep] && !registered[boxNum])
                                            || (sorted[scan] && !registered[boxNum]
                                                    && (a % 2 == 0 ? boxStep % 9 == scan % 9
                                                            : boxStep / 9 == scan / 9))) {
                                        grid[scan] = boxNum;
                                        grid[boxStep] = num;
                                        registered[boxNum] = true;
                                        continue ROW_COL;
                                    } else if (z == 8) // if z == 8, then break statement not reached: no candidates
                                                       // available
                                    {
                                            ...
                                            SWAP: for (int b = 0; b <= j; b++) {
                                                ...
                                                if (grid[pacing] == searchingNo) {
                                                    ...
                                                    for (int c = 1; c < 3 - (i % 3); c++) {
                                                        ...
                                                        // this creates the preference for swapping with unregistered
                                                        // numbers
    ...
    /**
     * Returns a formatted string of the sudoku grid
     * Comma seperated numbers in a row, with every row on a new line
     * 
     * @param int array - grid
     * @return String - formattedGrid
     */
